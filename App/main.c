...
//        rt_thread_startup(&keys);
	    ...
	    // 初始化 timer 
    result = rt_thread_init(&timers, "timers",      /* 线程名：timers */
        timers_entry, RT_NULL,                     	/* 线程的入口是timers_entry */
        &timers_stack[0], sizeof(timers_stack),   	/* 线程栈是timers_stack */
        THREAD_PRIORITY - 2, 10);
    if (result == RT_EOK)                           /* 如果返回正确，启动线程2 */
        rt_thread_startup(&timers);
    return 0;
}
